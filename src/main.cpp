...
#if defined(NDEBUG)
# error "Sojourn cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x36e7ad05a3ea9f58494714fda607235085860bdd7dbd5b70a33c537201781918");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sojourn: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sojourn Signed Message:\n";
...
{
    // Sojourn: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (pindexLast->nTime > 1505394793)	// Sept 14 2017, 13:13:13 GMT
    ...
    // Sojourn: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sojourn: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1500104109, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Trump Tower meeting included more people beyond Russian lawyer";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f189620bda11a24436b44b07afea1cc5f607e8472ff4e94d4736f5d8156cad8539cf786f7b08d687e0206f2a769de0d26bce44a1d0954895cdb786db33eda431") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1500104109;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2596460;
        ...
        assert(block.hashMerkleRoot == uint256("0xe7f4f656ee61a706ce21fdfef53dcde5bc104177d9330ceab08f2efa2aa2975a"));
...
unsigned char pchMessageStart[4] = { 0x70, 0x30, 0x7a, 0xd1 }; // Sojourn: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SojournMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SojournMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SojournMiner(CWallet *pwallet)
{
    printf("SojournMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sojourn-miner");
    ...
    try { loop {
        ...
        printf("Running SojournMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SojournMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SojournMiner, pwallet));
}
